...
export default function CreateEditTaskForm({ defaultCategory = "now", handleCreateTask, handleUpdateTask, onClose, task }: CreateEditTaskForm) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (task) {
      ...
      handleCreateTask(parentText, category, cleanedSubtasks);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-lg overflow-hidden">
      ...
      {(showSubtaskForm || subtasks.length > 0) ? (
        <div className="px-3 pt-2 pb-3">
          <div className="bg-[var(--color-surface-elevated-2)] border border-[var(--color-border)] rounded-lg">
            ...
            <div className="flex items-center justify-between px-3 py-2 border-b border-[var(--color-border)]">
              ...
              {!showSubtaskForm && (
                <Button onClick={() => setShowSubtaskForm(true)} type="button" size="xsmall" variant="secondary">Add subtask</Button>
              )}
            </div>

            {subtasks.map((subtask, index) => (
              <div key={index} className="flex items-center gap-2 px-3 min-h-[44px]">
                <input
                  type="text"
                  value={subtask}
                ...
                />
                <Button onClick={() => removeSubtask(index)} ariaLabel="Remove subtask" type="button" icon={<SvgClose />} size="small" variant="ghost" />
              </div>
            ))}
            ...
            {showSubtaskForm && (
              <>
                ...
                <div className="flex gap-2 items-center justify-end px-3 py-2">
                  <Button onClick={() => setShowSubtaskForm(false)} type="button" variant="secondary">Cancel</Button>
                  <Button onClick={(e) => saveSubtasks(e)} type="button" variant="primary">Save subtask{subtasks.length > 1 && "s"}</Button>
                </div>
              </>
            )}
        ...
        <div className="p-2">
          <Button onClick={() => setShowSubtaskForm(true)} icon={<SvgAdd />} type="button" variant="secondary" size="small">Add sub-task</Button>
        </div>
      )}
      ...
      {!showSubtaskForm && (
        <div className="flex items-center gap-4 justify-between px-3 py-2 border-t border-[var(--color-border)]">
          ...
          <div className="flex gap-2 items-center justify-end">
            ...
            <Button ref={submitButtonRef} type="submit" variant="primary">{`${task ? "Update" : "Add"} task`}</Button>
          </div>
        </div>
      )}
    </form>
